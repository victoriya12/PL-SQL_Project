...
procedure cust_limitBooks(p_user_name lib_customers.username%TYPE) is
    lim number;
    v_count number;
  begin
    ...
    select count(c.username)
     ...
     where --r.status NOT in ('TAKEN', 'RETURNED', 'CANCELED') AND
...
procedure lib_status is
  ...
  begin
    ...
    update lib_requests r
       set r.status = 'CANCELED', r.user_change = 'SYSTEM'
     where r.status = 'NEW'
...
procedure lib_showAll is
    ...
    cursor c_authors is
      select i.description from lib_nom_items i where i.group_id = 1;
  
    type c_list is table of lib_nom_items.description%TYPE;
  ...
  begin
    ...
    for i in c_authors loop
      counter := counter + 1;
      name_list.extend;
      name_list(counter) := i.description;
...
procedure rent_book(p_user_name      in lib_customers.username%TYPE,
    ...
    not_enough_books_left exception;
 ...
 <<break_procedure>>
  ...
  exception
    ...
    when not_enough_books_left then
