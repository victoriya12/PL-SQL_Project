...
alter table LIB_BOOKS
  add constraint LIB_BOOKS_AUTHORS_FK foreign key (AUTHOR_ID)
  references LIB_AUTHORS (ID);
alter table LIB_BOOKS
  add constraint LIB_BOOKS_GENRE_FK foreign key (GENRE_ID)
  references LIB_GENRE (ID);
...
alter table LIB_AUTHORS
  add constraint LIB_AUTHORS_PK primary key (ID)


create table LIB_AUTHORS_HISTORY
(
  history_id  NUMBER default "C##VIKI"."LIB_AUTHORS_HISTORY_SEQ"."NEXTVAL" not null,
  log_date    DATE,
  id          NUMBER not null,
  name        VARCHAR2(100),
  user_change VARCHAR2(20),
...
alter table LIB_GENRE
  add constraint LIB_GENRE_PK primary key (ID)

create table LIB_GENRE_HISTORY
(
  history_id  NUMBER default "C##VIKI"."LIB_GENRE_HISTORY_SEQ"."NEXTVAL" not null,
  log_date    DATE,
  id          NUMBER not null,
  description VARCHAR2(100),
  user_change VARCHAR2(20),
...
create or replace trigger lib_authors_trg_buid
  before insert or update or delete on lib_authors
  for each row
declare
  -- local variables here
begin
  if inserting or updating then
    :new.date_change := sysdate;
    :new.user_change := user;
  end if;
  if inserting then
    :new.id := lib_authors_seq.nextval;
  elsif updating or deleting then
    insert into lib_authors_history
      (log_date, id, name, user_change, date_change)
    values
      (sysdate, :old.id, :old.name, :old.user_change, :old.date_change);
  end if;
end lib_authors_trg_buid;


create or replace trigger lib_genre_trg_buid
  before insert or update or delete on lib_genre
  for each row
declare
  -- local variables here
begin
  ...
  if inserting then
    :new.id := lib_genre_seq.nextval;
  elsif updating or deleting then
    insert into lib_genre_history
      (log_date, id, description, user_change, date_change)
    values
      (sysdate,
       :old.id,
       :old.description,
